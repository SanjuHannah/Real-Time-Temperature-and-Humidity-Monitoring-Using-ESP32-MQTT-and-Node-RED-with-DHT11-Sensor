//code for Real-Time Temperature and Humidity Monitoring Using ESP32, MQTT, and Node-RED with DHT11 Sensor(HW id:H00485847)
#include <WiFi.h>
#include <DHT.h>
...
#define MQTT_DEVICEID "d:hwu:esp8266:H00485847"
#define MQTT_USER ""
#define MQTT_TOKEN ""
#define MQTT_TOPIC "d:hwu:esp8266:H00485847/evt/status/fmt/json"
#define MQTT_TOPIC_DISPLAY "d:hwu:esp8266:H00485847/cmd/display/fmt/json"
#define MQTT_TOPIC_INTERVAL "d:hwu:esp8266:H00485847/cmd/interval"
#define MQTT_TOPIC_LED_CONTROL "d:hwu:esp8266:H00485847/cmd/led_control"
...
void loop() {
  ...
  if (millis() - lastPublishTime >= PUBLISH_INTERVAL) {
    ...
    if (isnan(h) || isnan(t)) {
      ...
      if (!manualLEDControl) {
        if (t <= ALARM_COLD) {
          setLED(255, 255, 0);  // Blue for cold alarm
        } else if (t <= WARN_COLD) {
          setLED(150, 150, 0);  // Blue for cold warning
        } else if (t >= ALARM_HOT) {
          setLED(0, 255, 255);  // Red for hot alarm
        } else if (t >= WARN_HOT) {
          setLED(0, 150, 150);  // Red for hot warning
        } else {
          setLED(255, 0, 255);  // Green for normal
        }
      }
